...
1. **Gestion des terrains** (RG1-RG7) : Structure de base présente avec le module `land`
2. **Gestion des espaces de culture** (RG8-RG14) : Module `cultivation-space` existant
3. **Gestion des cultures** (RG22-RG28, RG30) : Module `crop` implémenté avec le statut
4. **Gestion des utilisateurs** (RG47-RG49) : Fonctionnalités de base avec `user` et `role`
...
2. **Module d'actions agricoles** (RG31-RG41) :
   - Créer une entité `AgriculturalAction` avec type, date, commentaire
   - Établir des relations avec `CultivationSpace`, `CultivationBed` et `Crop`
   - Implémenter un calendrier d'actions
...
2. Implémenter le module d'actions agricoles qui représente le cœur fonctionnel
